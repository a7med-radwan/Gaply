...
<body class="h-full flex flex-col lg:flex-row bg-darkBg text-white antialiased">
    ...
    <!-- MOBILE TOP HEADER -->
    <header class="lg:hidden w-full flex items-center justify-between px-6 py-4 bg-darkCard border-b border-darkBorder sticky top-0 z-40">
        <a href="/" class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-xl flex items-center justify-center text-white font-black text-sm bg-gradient-to-br from-oceanBlue to-[#1e40af]">
                G
            </div>
    ...
    <!-- SIDEBAR NAVIGATION -->
    <aside id="sidebar" class="w-64 bg-darkCard border-r border-darkBorder/40 flex flex-col justify-between fixed inset-y-0 left-0 z-30 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out lg:static lg:h-screen">
        
        <div class="flex flex-col gap-8 py-6 px-6">
            <!-- Brand Logo -->
            <div class="flex items-center justify-between">
                <a href="/" class="flex items-center gap-3 group">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center text-white font-black text-base bg-gradient-to-br from-oceanBlue to-[#1e40af] shadow-md shadow-oceanBlue/20">
                        G
                    </div>
            ...
            <nav class="space-y-1.5">
                <p class="text-[9px] font-mono font-bold uppercase tracking-widest text-textSecondary/50 mb-3 px-3">Navigation</p>
                ...
                <a href="{{ route('tasks.index') }}" 
                   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 group {{ request()->routeIs('tasks.*') ? 'bg-oceanBlue text-white shadow-premium' : 'text-textSecondary hover:text-white hover:bg-darkBg/50' }}">
        ...
        <div class="p-4 border-t border-darkBorder/40 bg-darkBg/20 space-y-4">
            <div class="flex items-center gap-3 px-2">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-[10px] text-textSecondary truncate">{{ auth()->user()->email }}</p>
                </div>
            </div>

            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold border border-darkBorder text-textSecondary hover:text-white hover:border-red-500/40 hover:bg-red-500/5 transition-all">
    ...
    <div class="fixed bottom-6 right-6 z-50 flex flex-col gap-3 max-w-sm w-full pointer-events-none">
        ...
        {{-- Success Toast --}}
        @if (session('success'))
            <div id="toast-success" class="pointer-events-auto flex items-start gap-3 p-4 rounded-2xl border border-accentTeal/20 bg-darkCard/90 text-sm shadow-2xl shadow-accentTeal/5 border-l-4 border-l-accentTeal animate-slide-in">
                ...
                <button onclick="document.getElementById('toast-success').remove()" class="text-textSecondary hover:text-white transition-colors">
        ...
        {{-- Status Toast --}}
        @if (session('status'))
            <div id="toast-status" class="pointer-events-auto flex items-start gap-3 p-4 rounded-2xl border border-oceanBlue/20 bg-darkCard/90 text-sm shadow-2xl shadow-oceanBlue/5 border-l-4 border-l-oceanBlue animate-slide-in">
                ...
                <button onclick="document.getElementById('toast-status').remove()" class="text-textSecondary hover:text-white transition-colors">
                    <span class="material-symbols-outlined text-base">close</span>
                </button>
            </div>
        @endif
        ...
        @if ($errors->any() && !request()->routeIs('login') && !request()->routeIs('register') && !request()->routeIs('password.*'))
            <div id="toast-error" class="pointer-events-auto flex items-start gap-3 p-4 rounded-2xl border border-red-500/20 bg-darkCard/90 text-sm shadow-2xl shadow-red-500/5 border-l-4 border-l-red-500 animate-slide-in">
                ...
                <button onclick="document.getElementById('toast-error').remove()" class="text-textSecondary hover:text-white transition-colors">
                    <span class="material-symbols-outlined text-base">close</span>
                </button>
            </div>
        @endif
